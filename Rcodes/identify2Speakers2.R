...
identify2Speakers2<-function(SpeechUrl,
speakerID=c('Rcodes/digits/喜出望外.wav','Rcodes/digits/自强不息.wav'),diffGender=TRUE, speaker1.sound=NULL,speaker2.sound=NULL){
...
speaker1wav=suppressWarnings(tuneR::readWave(speakerID[1]))
if(speaker1wav@samp.rate!=sep.values$samp.rate){
speaker1wav=seewave::resamp(speaker1wav,speaker1wav@samp.rate,sep.values$samp.rate,"Wave")
}
speaker1wav@stereo=FALSE;
speaker1wav@right=numeric(0)

speaker2wav=suppressWarnings(tuneR::readWave(speakerID[2]))
if(speaker2wav@samp.rate!=sep.values$samp.rate){
speaker2wav=seewave::resamp(speaker2wav,speaker2wav@samp.rate,sep.values$samp.rate,"Wave")
}
speaker2wav@stereo=FALSE;
speaker2wav@right=numeric(0)

silenceSound=speaker1wav;
silenceSound@left=c(1:round(0.1*sep.values$samp.rate))*0;#0.1秒静音
speaker1wav=tuneR::bind(silenceSound,speaker1wav,silenceSound);#前后添加0.1秒静音
speaker2wav=tuneR::bind(silenceSound,speaker2wav,silenceSound);
