...
processFolder2 <- function(folderName) {
  # Start with empty data.frame.
  data <- data.frame()

  # Get list of subfolders in the folder.
  folderlist=list.files(folderName);

  for (subfolderName in folderlist){
   subfolder=paste(folderName,'/',subfolderName,sep="")
   acoustics <- processFolder(subfolder)
   data=rbind(acoustics,data)
   }
  data
}
